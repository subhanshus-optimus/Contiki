...
{
  int len = snprintf(pub_topic, BUFFER_SIZE, "zolertia/%s",
                     conf.event_type_id);
...
{
  int len = snprintf(sub_topic, BUFFER_SIZE, "zolertia/%s",
                     conf.cmd_type);
  if(len < 0 || len >= BUFFER_SIZE) {
...
static int init_config()
{
  ...
  memcpy(conf.event_type_id, "status", 
         strlen("status")); //DEFAULT_EVENT_TYPE_ID
  memcpy(conf.broker_ip, broker_ip, strlen(broker_ip));
  memcpy(conf.cmd_type, "leds", 4);  //DEFAULT_SUBSCRIBE_CMD_TYPE
...
static void publish(void)
{
  ...
  seq_nr_value++;
  buf_ptr = app_buffer;
  int random_number=random_rand();

  len = snprintf(buf_ptr, remaining,
                 "Random number: %d",random_number);
